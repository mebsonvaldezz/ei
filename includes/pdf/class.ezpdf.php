...
class Cezpdf extends Cpdf
{
	...
	function Cezpdf($paper = 'A4', $orientation = 'portrait')
	{
		...
		if (!is_array($paper))
		{
			switch (strtoupper($paper))
			{
				...
				case 'A4':	$size = array(0,0,595.28,841.89);			break;
				...
				case 'LETTER': default: $size = array(0,0,612.00,792.00); break;
			}
		...
		$this->y = $this->ez['pageHeight'];
		
		// Get the ID of the page that was created during the instancing process.
	...
	function ezColumnsStart($options = array())
	{
		...
		// Store the settings for the columns
	...
	function ezNewPage()
	{
		...
		if (isset($this->ez['columns']) && $this->ez['columns']['on'] == 1)
		{
			// Check if this is just going to a new column increment the column number
	...
	function ezSetMargins($top, $bottom, $left, $right)
	{
		...
		// Check to see if this means that the current writing position is outside the writable area
	...
	function ezStartPageNumbers($x, $y, $size, $pos = 'left', $pattern = 'Página {PAGENUM} de {TOTALPAGENUM}', $num = '')
	{
		...
		$i = sizeof($this->ez['pageNumbering']);
	...
	function ezStopPageNumbers($stopTotal = 0, $next = 0, $i = 0)
	{
		...
		if ($next && isset($this->ez['pageNumbering'][$i][$this->ezPageCount]) && is_array($this->ez['pageNumbering'][$i][$this->ezPageCount]))
		{
			...
			$key = ($stopTotal) ? 'stoptn' : 'stopn';
			$this->ez['pageNumbering'][$i][$this->ezPageCount][$key] = 1;
		}
		else
		{
			$key = ($stopTotal) ? 'stopt' : 'stop';
			$this->ez['pageNumbering'][$i][$this->ezPageCount] = $key . (($next) ? 'n' : '');
		}
	}
	...
	function ezPrvtTableDrawLines($pos, $gap, $x0, $x1, $y0, $y1, $y2, $col, $inner, $outer, $opt = 1)
	{
		...
		// Only do the second line if it is different to the first, AND each row does not have a line on it.
		if ($y0 != $y1 && $opt < 2)
		{
	...
	{	
		// Will calculate the maximum width, taking into account that the text may be broken by line breaks.
		$mx = 0;
		$lines = explode("\n", $text);
	...
	function ezTable(&$data, $cols = '', $title = '', $options = '')
	{
		...
		if ($options['width'] && $adjustmentWidth > 0 && $setWidth<$options['width'])
		{
			...
			{
				$s_cols0 = sizeof($cols0);
				foreach ($cols0 as $colName => $w)
				{
					$cols0[$colName] += ($neededWidth - $presentWidth) / $s_cols0;
				}
			}
			else
			{
				...
				while ($presentWidth > $neededWidth && $cnt < 100)
				{
					...
					$newWidestWidth = $widest - ($presentWidth - $neededWidth) / sizeof($aWidest);
					...
					else
					{
						...
						$presentWidth = $presentWidth - ($widest - $nWidest) * sizeof($aWidest);
					}
				}
			} // else
	...
	{
		// this function will intially be used to implement underlining support, but could be used for a range of other purposes
	...
	function ezImage($image, $pad = 5, $width = 0, $resize = 'full', $just = 'center', $border = '')
	{
		...
		if ($type == "jpeg")
		{
			$this->addJpegFromFile($image, $this->ez['leftMargin'], $this->y - $height, $width);
		}
	...
	function alink($info, $internal = 0)
	{
		...
		switch ($info['status'])
		{
			...
			case 'end':
			case 'eol':
				// The end of the link assume that it is the most recent opening which has closed
				$i = $info['nCallback'];
